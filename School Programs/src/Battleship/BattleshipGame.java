...
public class BattleshipGame {
		...
		for (int x = 1; x < 6; x++) {
			...
			do {
				...
				if (choice < 1 && choice > 2) {
					...
					if (choice == 1)
						orientation = true; // true for horizontal, false for
											// vertical
					else if (choice == 2)
						orientation = false; // true for horizontal, false for
												// vertical
				}
	...
	private boolean checkShipPlacement(boolean orin, int length, int cordX, int cordY, String player) {
		...
		if (orin) {
			if (cordX + length > 11) return false;
			if (cordY > 11) return false;
		} else if (!orin) {
			if (cordX > 11) return false;
			if (cordY + length > 11) return false;
		}

		if (player.equals("player")) {
			for (int i = length; i > 0; i--) {
				if (orin && playerShips[cordX + i - 1][cordY]) return false;
				if (!orin && playerShips[cordX][cordY + i - 1]) return false;
			}
		}
		else if (player.equals("cpu")) {
			for (int i = length; i > 0; i--) {
				if (orin && cpuShips[cordX + i - 1][cordY]) return false;
				if (!orin && cpuShips[cordX][cordY + i - 1]) return false;
			}
		}
		return true;
	}
