...
public class BattleshipGame {
		...
		for (int x = 1; x < 6; x++) {
			...
			do {
				...
				if (choice < 1 && choice > 2) {
					...
					if (choice == 1) orientation = true; // true for horizontal, false for
															// vertical
					else if (choice == 2) orientation = false; // true for horizontal, false for
																// vertical
				}
				...
				if (!redo) {
					...
					redo = !game.checkShipPlacement(orientation, shipLength[x], cordX, cordY);
				}
	...
	private void cpuShipPlacement() {
		...
		for (int x = 1; x < 6; x++) {
			...
			do {
				...
				redo = !game.checkShipPlacement(orin, shipLength[x], cordX, cordY);
			} while (redo);
			...
			for (int i = shipLength[x]; i > 0; i--) {
				...
				if (orin) playerShips[cordX + (shipLength[x] - i)][cordY] = true;
				else if (!orin) playerShips[cordX][cordY + (shipLength[x] - i)] = true;
			}
		}
	}
	...
	private boolean checkShipPlacement(boolean orin, int length, int cordX, int cordY) {
		...
		for (int i = length; i > 0; i--) {
			if (orin && playerShips[cordX + i][cordY]) return false;
			if (!orin && playerShips[cordX][cordY + i]) return false;
		}
		return true;
	}
	...
	private void printBoard() {
		...
		for (int y = 1; y < 11; y++) {
			...
			for (int x = 1; x < 11; x++) {
				...
				if (y == 10 && x == 1) System.out.print("" + space);
				else System.out.print(" " + space);
			}
			
			System.out.print("\t" + y);
			// ///////////////////////Printing out CPU ships for testing purposes
			for (int x = 1; x < 11; x++) {
				...
				if (y == 10 && x == 1) System.out.print(" " + space);
				else if (x == 1) System.out.print("  " + space);
				else System.out.print(" " + space);
			}
			System.out.print("\n");
		}
